...
Worth noting for the v1/v2 scope question: the S3 serving its own
status page covers a chunk of what the Pi dashboard did, **on the
ESP32**. For the in-car always-on build WiFi would normally stay off to
save power, so this is a bench/dev convenience today — but it's a hint
that v1 may not need the Pi even for a glanceable status page.

## Divergence from the documented design (flagging, not fixing)

Today's bench hardware differs from what `docs/BOM.md` and
`docs/architecture.md` currently describe. Logging it honestly rather
than silently rewriting the design docs off one bench session:

| Aspect | Docs say (v1 as written) | Built today |
|---|---|---|
...
None of this is wrong — it's arguably a better fit for the use case
(far lower quiescent draw; the S3's extra RAM/PSRAM makes the
self-served dashboard trivial). But it's a real fork from the BOM. Open
questions before reconciling the docs:

- Is the **onboard ADC accurate enough** after calibration, or does the
  ADS1115 come back for the final build? (The ESP32 ADC is famously
  nonlinear near the rails; `cal` + staying mid-scale may be enough.)
- Is the **S3 the permanent MCU choice** for v1, replacing the
  WROOM-32U in the BOM?

I'd rather answer those with another bench session or two than rewrite
`BOM.md` / `architecture.md` now. Offer stands to reconcile them once
the ADC-accuracy question is settled.

...
class Config:
    ...
    COMPUSTAR_PACKETS = _TEST_PACKETS
...
def test_pi_start_engine_triggers_full_path_and_emits_event():
    ...
    # Controller should have actually transmitted (since packets are populated)
...
def test_compustar_tx_event_has_button_in_detail():
    ...
    while True:
        m = pi_link.recv()
        if m is None: break
        if m.get("type") == "EVENT" and m.get("event") == "compustar_tx":
            tx_event = m
            break
    assert tx_event is not None
    detail = tx_event["detail"]
...
def test_event_constants_match_between_sides():
    ...
    for name in ("EVENT_ENGINE_STARTED", "EVENT_ENGINE_STOPPED",
                 ...
                 "EVENT_PI_SHUTDOWN_REQUEST", "EVENT_COMPUSTAR_TX",
                 "EVENT_COMPUSTAR_TX_FAIL"):
        assert getattr(esp_side, name) == getattr(pi_side, name), \
            f"{name} differs between esp32 and pi pi_link/esp32_link modules"
